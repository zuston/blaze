...
import org.apache.spark.sql.blaze.BlazeSparkSessionExtension.{blazeEnabledKey, dumpSimpleSparkPlanTreeNode}
...
class BlazeSparkSessionExtension extends (SparkSessionExtensions => Unit) with Logging {
  ...
  override def apply(extensions: SparkSessionExtensions): Unit = {
    ...
    extensions.injectQueryStagePrepRule { session => BlazeExecRule(session) }
  }
}
  ...
  override def preColumnarTransitions: Rule[SparkPlan] = {
    new Rule[SparkPlan] {
      override def apply(sparkPlan: SparkPlan): SparkPlan = {
        BlazeExecRule(sparkSession).apply(sparkPlan)
      }
    }
  }
}

case class BlazeExecRule(session: SparkSession) extends Rule[SparkPlan] {
  override def apply(sparkPlan: SparkPlan): SparkPlan = {
    ...
    // post-transform
    Shims.get.postTransform(sparkPlanTransformed, session.sparkContext)
    sparkPlanTransformed
  }
}
